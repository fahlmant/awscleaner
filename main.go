...
func main() {
	...
	//	fmt.Println("error in assuming role is ",err);
	...
	for _, region := range regionRange {
		...
		token := ""
		for {
			result2, err := client2.DescribeInstances(&ec2.DescribeInstancesInput{NextToken: aws.String(token)})
			if err != nil {
				fmt.Println("error is ", err)
			}
			fmt.Println("EC2 instances", result2)
			if result2.NextToken != nil {
				token = *result2.NextToken
			} else {
				break
			}
		}
